...
def test_lookup_exercise_returns_best_match():
    get_db().exercises.insert_many([
        _make_doc("barbell bench press", "chest"),
        _make_doc("incline dumbbell press", "chest"),
    ])
...
def test_lookup_exercise_404_below_threshold():
    get_db().exercises.insert_many([_make_doc("barbell bench press", "chest")])
...
def test_lookup_exercise_is_case_insensitive():
    get_db().exercises.insert_many([_make_doc("Barbell Curl", "biceps")])
...
def test_lookup_exercise_includes_instructions_and_attribution():
    ...
    get_db().exercises.insert_one(doc)
